...
public enum SFXType {
    ...
    UI_INV_CLOSE = 3003,
    WORLD_MOVE = 4000,
    WORLD_CANNOTMOVE = 4001,
    USEITEM_SUCCESS = 5000,
    USEITEM_FAILURE = 5001,
}
...
public class AudioManager : MonoBehaviour {
    ...
    public void PlayMusic (int index) { // for use with unity actions - references the index nr. of the music in the list
        if (index >= 0 && index < m_music.Length) {
            Debug.Log ("<color=green>Playing music at index " + index + "</color>");
            PlayMusic (m_music[index].m_clip);
        }
    }
    public void PlayMusic (AudioClip clip) { // simple simple
        PlayMusic (clip, true);
    }
    ...
    public void QueueMusic (MusicType musicToQueue, MusicType musicToPlayAfter) { // a simple 'queue music' type thing to play e.g. victory tunes
